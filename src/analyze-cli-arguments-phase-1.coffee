    ...
    for argument in argv
      ...
      switch type
        ...
        when 'other'
          ...
          throw new Error "Ωjsonick___5 should never happen: unknown lexeme type #{rpr type}"
    return R
  #---------------------------------------------------------------------------------------------------------
  _isa_null_pod = ( x ) -> ( Object.getPrototypeOf x ) is null
  #---------------------------------------------------------------------------------------------------------
  show_cdef = ( cdef ) ->
    fifo  = if cdef.f? then 'FIFO' else null
    cdef.c[ idx ] = { entry..., } for entry, idx in cdef.c when _isa_null_pod entry
    cdef.d[ idx ] = { entry..., } for entry, idx in cdef.d when _isa_null_pod entry
    echo '{'
    echo """"a":#{JSON.stringify cdef.a},"""
    echo """"c":#{JSON.stringify cdef.c},"""
    echo """"d":#{JSON.stringify cdef.d},"""
    echo """"e":#{JSON.stringify cdef.e},"""
    echo """"f":#{JSON.stringify fifo}"""
    # echo rpr cdef
    # echo rpr cdef
    echo '}'
    ;null
